...
def loadImages(path, size):
    ...
    """Converts the image to a format that supports transparency (alpha channel). 
    This is useful for images with transparent areas, such as PNG files.
    The convert_alpha() method ensures that the image maintains its transparency information."""
...
#  Classes
class Othello:
    """ defined with an __init__ method that serves as the constructor. """
    def __init__(self):
        pygame.init()
...
class Grid:
    ...
    def endScreen(self):
        if self.GAME.gameOver:
            ...
            wonText = self.font.render(f'{" Congratulations, You Won!" if (self.player1Score > self.player2Score) else " "}', 1, 'White')
            endScreenImg.blit(wonText, (0, 0))
            lossText = self.font.render(f'{" Bad Luck, You Lost" if (self.player1Score < self.player2Score) else " "}', 1, 'White')
            endScreenImg.blit(lossText, (0, 0))
            newGame = pygame.draw.rect(endScreenImg, 'White', (80, 160, 160, 80))
            GameText = self.font.render(f'{"Play Again" if ((self.player1Score < self.player2Score) or (self.player1Score > self.player2Score)) else "Start Playing"}', 1, 'Black')
            endScreenImg.blit(GameText, (120, 190))     
        return endScreenImg
    ...
    def drawGrid(self, window):
        ...
        window.blit(self.drawScore('Player 1, White', self.player1Score), (820, 100))
        window.blit(self.drawScore('AI Player, Black', self.player2Score), (820, 200))
    ...
    def insertToken(self, grid, currentplayer, y, x):
        tokenImage = self.whitetoken if currentplayer == 1 else self.blacktoken
